...
dogfood-docker: release
	docker build --build-arg TARGET=$(MUSL_TARGET) --tag unreferenced_files --file Dockerfile .
	docker run --rm --volume $(PWD):/workspace --workdir /workspace $(GITHUB_ACTIONS_ENV) unreferenced_files --help

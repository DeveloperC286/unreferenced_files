Feature: Only search for files that match any of the provided only-search-for regexes.


  Scenario Outline: The singular unreferenced file does not match any of the only search for regexes, so now there are no unreferenced files.
    ...
    Examples:
      | repository                                               | checkout_commit                          | search | search_for   | unreferenced_files              | only_search_for |
  ...
  Scenario Outline: Some of the unreferenced files do not match any of the only search for regexes, so now there are fewer unreferenced files.
    Given the repository "<repository>" is cloned and checked out at the commit "<checkout_commit>".
    When the argument --search is provided as "<search>".
    And the argument --search-for is provided as "<search_for>".
    And the flag --assert-no-unreferenced-files is set.
